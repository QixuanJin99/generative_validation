...
class Waterbirds_Dataset_Transfer(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, split, root):
        ...
        split_data = torch.utils.data.ConcatDataset([extra_ds1, extra_ds2])

        train_num = int(self.config.name)
        train_num = min(train_num, len(split_data))
        
        # Random select indices 
        random.seed(0)
        indices = random.sample(range(len(split_data)), k=train_num)
        ...
        for idx in indices: 
            ...
            if y == 0: 
                ...
                bird_label = "waterbird"
            prompt = f"a photo of {bird_label} with background of <wb-target-domain>"
